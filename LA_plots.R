...
render_points(extent = attr(elevation,"extent"),
              lat = st_coordinates(tmp2)[,2], long = st_coordinates(tmp2)[,1],
              altitude = (tmp2$Elevation)+10, zscale=1,   # zscale must match that of the above (its scales the exageration of elevation) 
              color="red", size=6)
...
# Dardo
# Subset to the locations you want to plot
locs <- all.locs[all.locs$Group=="Dardo",]
locs<- locs[locs$Project=="Grids",]
# Convert them to sf
tmp2 <- st_as_sf(locs,coords=c("Longitude", "Latitude"), crs=4326)
...
render_highquality(filename="Plots/Dardo_render.png", # The file name you want!
                   # Light controls - if you supply multiple values you get multiple lightsources
                   lightdirection = c(-45,45), lightaltitude  = 35, clamp_value = 10,
                   samples = 400, 
                   # Camera locations
